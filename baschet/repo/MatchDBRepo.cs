...
public class MatchDBRepo: MatchRepository
{
    ...
    public Match findOne(int id)
    {
        ...
        {
            comm.CommandText = "select id,teamA,teamB,ticketPrice, soldSeats from Matches where id=@id";
            ...
            using (var dataR = comm.ExecuteReader())
            {
                if (dataR.Read())
                {
                    ...
                    int soldSeats = dataR.GetInt32(4);
    ...
    public IEnumerable<Match> findAll()
    {
        ...
        using (var comm = con.CreateCommand())
        {
            comm.CommandText = "select id,teamA,teamB,ticketPrice, soldSeats from Matches";
    ...
    public Match add(Match entity)
    {
        ...
        using (var comm = con.CreateCommand())
        {
            comm.CommandText = "insert into Matches(teamA,teamB,ticketPrice, soldSeats) values (@teamA,@teamB,@ticketPrice,@totalSeats, @soldSeats)";
    ...
    public void update(Match entity)
    {
        ...
        using (var comm = con.CreateCommand())
        {
            comm.CommandText = "update Matches set teamA=@teamA, teamB=@teamB, ticketPrice=@ticketPrice, soldSeats=@soldSeats where id=@id";
            var paramId = comm.CreateParameter();
            paramId.ParameterName = "@id";
    ...
    public Boolean CheckAvailableSeats(Match entity, int seats)
    {
        if (seats == 0)
        {
            if (entity.SoldSeats - seats > 0)
            {
                return true;
            }
            return false;
